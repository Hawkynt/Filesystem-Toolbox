  ...
  internal static unsafe class BlockComparer {
    ...
    public static bool IsEqual(byte[] source, int sourceLength, byte[] comparison, int comparisonLength) {
      ...
      fixed (byte* sourcePin = source, comparisonPin = comparison) {
        ...
        return
            _CompareUInt64(&currentSource, &currentComparison, ref remainingLength)
            && _CompareUInt32(&currentSource, &currentComparison, ref remainingLength)
            && _CompareBytes(&currentSource, &currentComparison, ref remainingLength)
          ;
      }
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static bool _CompareUInt64(byte** source, byte** comparison, ref int count) {
      var s = (long*)*source;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static bool _CompareUInt32(byte** source, byte** comparison, ref int count) {
      var s = (int*)*source;
